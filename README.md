...
| `scad/fit_test.scad` | Coupon for finding the spline press fit: sockets at several clearances plus loose plugs. |
...
| `measure/` | Per version (`v1/`, `v2/`): annotated photos with lettered callouts plus a table of what to measure. |
...
with the number engraved beside it, plus loose plugs at nominal. It prints in
the same orientation as the real parts, sockets as holes in an upright boss and
plugs standing on the bed, and it cuts the same geometry the cam and the gear
use rather than a copy. Print it with the settings you will use for the parts,
press a plug into each socket, and keep the clearance that grips without
splitting. Edit `ft_clearances` to change the range.
